...
============================================================
1) FIX bug memoria: build_eval_dataset() ora carica UNA SOLA VOLTA gli
   array 'embeds' e 'paths' dal file .npz (embed_cache_path) prima di
   costruire il dizionario, invece di indicizzare data["embeds"][i]
   dentro il dict comprehension. Nella versione originale ogni accesso
   a data["embeds"] ri-decomprime e ricostruisce l'INTERO array dallo
   zip .npz (NpzFile non fa caching), quindi con migliaia di campioni
   l'array veniva riallocato migliaia di volte -> frammentazione heap
   -> ArrayMemoryError anche per allocazioni piccole (es. "Unable to
   allocate 14.5 MiB").

2) Modalita' 'single' piu' onesta di default: se non passi
   --eval_subjects_file, invece di usare TUTTI i soggetti di --data_dir
   (che include quelli visti in training = risultato gonfiato), lo
   script prova a derivare i soggetti di VALIDATION dal checkpoint
   stesso: soggetti presenti in --data_dir MA ASSENTI da
   ckpt["subject_to_label"] (il modello finale non ha mai visto questi
   soggetti come classi in training, quindi sono un test piu' onesto).
   Se il checkpoint e' stato allenato su TUTTI i soggetti disponibili
   (insieme vuoto), lo script avvisa chiaramente e ripiega sul
   comportamento originale (tutti i soggetti, risultato da trattare
   solo come sanity check, non come metrica open-set).
...
python evaluate_rank1.py --stream palm \
    ...
    --mode nested_cv \
    --embed_cache_path vit_cache.npz
...
python evaluate_rank1.py --stream dorsal \
    ...
    --mode nested_cv \
    --embed_cache_path swin_cache.npz

# Modello FINALE dorso, sui soggetti esclusi dal training (derivati
# automaticamente da subject_to_label nel checkpoint)
python evaluate_rank1.py --stream dorsal \
    --data_dir dataset_preprocessed \
    --checkpoint models_final_dorsal/dorsal_embedding_final.pt \
    --mode single \
    --embed_cache_path swin_cache.npz

# Modello FINALE palmo, stesso principio
python evaluate_rank1.py --stream palm \
    --data_dir dataset_preprocessed \
...
# Se preferisci passare TU la lista di soggetti (es. un file gia'
# salvato in precedenza), --eval_subjects_file ha sempre la precedenza:
python evaluate_rank1.py --stream dorsal \
    --data_dir dataset_preprocessed \
    --checkpoint models_final_dorsal/dorsal_embedding_final.pt \
    --mode single \
    --eval_subjects_file dorsal_val_subjects.txt \
    --embed_cache_path swin_cache.npz
============================================================
"""
...
def build_eval_dataset(stream: str, data_dir, eval_subjects, n_knuckles_max,
    ...
    if stream == "palm":
        ...
        if embed_cache_path:
            ...
            # FIX: carica gli array UNA SOLA VOLTA prima del dict comprehension.
            # data["embeds"]/data["paths"] su un NpzFile ri-decomprime l'intero
            # array ad ogni accesso (nessun caching interno): indicizzarli
            # dentro un loop item-per-item causava riletture ripetute
            # dell'intero array e frammentazione dell'heap -> ArrayMemoryError.
            embeds = data["embeds"]
            paths = data["paths"]
        ...
        if embed_cache_path:
            data = np.load(embed_cache_path, allow_pickle=True)
            # FIX: vedi commento sopra, stesso bug nel branch dorsal.
            embeds = data["embeds"]
            paths = data["paths"]
...
# ============================================================
# DERIVAZIONE SOGGETTI DI VALIDATION DAL CHECKPOINT (modalita' single)
# ============================================================
def derive_val_subjects_from_checkpoint(checkpoint_path, stream: str, data_dir):
    """
    Se il checkpoint finale non ha 'outer_test_subjects' (tipico dei modelli
    finali, non nested_cv), prova a derivare un test set piu' onesto:
    soggetti presenti in --data_dir ma ASSENTI da ckpt['subject_to_label']
    (mai visti come classe in training, quindi mai usati per aggiornare i
    pesi). Se l'insieme risulta vuoto, il modello e' stato allenato su
    TUTTI i soggetti disponibili: in quel caso ritorna None e chi chiama
    deve ripiegare sul comportamento originale (tutti i soggetti, da
    trattare come sanity check, non come metrica open-set onesta).
    """
    from importlib import import_module
    core = import_module("palm_core" if stream == "palm" else "dorsal_core")

    ckpt_probe = torch.load(checkpoint_path, map_location="cpu")
    train_subjects = set(ckpt_probe.get("subject_to_label", {}).keys())
    all_subjects = set(core.list_subjects(data_dir))
    val_subjects = sorted(all_subjects - train_subjects)

    print(f"Soggetti totali in data_dir: {len(all_subjects)}")
    print(f"Soggetti visti in training (subject_to_label): {len(train_subjects)}")
    print(f"Soggetti ESCLUSI dal training (candidati per l'eval): {len(val_subjects)}")

    if not val_subjects:
        print(
            "ATTENZIONE: nessun soggetto escluso dal training risulta in "
            "data_dir - il modello finale e' stato allenato su TUTTI i "
            "soggetti disponibili. Non esiste un test set open-set per "
            "questo checkpoint: il rank-1/rank-5 calcolato su tutto "
            "data_dir e' SOLO un sanity check, non una metrica open-set."
        )
        return None
    return val_subjects
...
def main():
    ...
    parser.add_argument(
        ...
        help="file di testo con un subject_id per riga; se omesso in modalita' "
             "'single', lo script prova a derivare i soggetti ESCLUSI dal "
             "training dal checkpoint stesso (subject_to_label); se il "
             "modello e' stato allenato su tutti i soggetti disponibili, "
             "usa TUTTI i soggetti in --data_dir come sanity check.",
    )
    ...
    if args.mode == "nested_cv":
        ...
        if args.eval_subjects_file:
            ...
            if "outer_test_subjects" in ckpt_probe:
                eval_subjects_override = None  # lascia che evaluate_one_checkpoint lo prenda dal checkpoint
            else:
                # NUOVO: prova prima a derivare i soggetti esclusi dal
                # training (test set piu' onesto del "tutti i soggetti").
                eval_subjects_override = derive_val_subjects_from_checkpoint(
                    args.checkpoint, args.stream, args.data_dir
                )
                if eval_subjects_override is None:
                    from importlib import import_module
                    core = import_module("palm_core" if args.stream == "palm" else "dorsal_core")
                    eval_subjects_override = core.list_subjects(args.data_dir)
                    print(
                        "Uso TUTTI i soggetti di --data_dir come sanity check "
                        "(vedi avviso sopra: il risultato NON e' open-set/onesto)."
                    )
